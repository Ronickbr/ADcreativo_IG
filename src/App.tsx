...
function ImageInput({ id, value, label, hint, onChange }: { id?: string; value: ImageState; label: string; hint: string; onChange: (image: ImageState) => void }) {
    ...
    <div>
      ...
      <div className="relative">
        <button
          ...
          className="upload-zone group w-full"
          aria-label={value.preview ? `Alterar ${label.toLowerCase()}` : `Enviar ${label.toLowerCase()}`}
        >
          {value.preview ? (
            <img src={value.preview} alt={`Pré-visualização de ${label.toLowerCase()}`} className="h-full w-full object-contain p-3" />
          ) : (
            <>
              <span className="icon-box"><ImagePlus size={20} /></span>
              <span><b>Enviar imagem</b><small>PNG, JPG ou WebP</small></span>
            </>
          )}
        </button>
        {value.preview && (
          <button
            ...
            className="absolute right-2 top-2 rounded-full bg-black/70 p-1.5 text-white transition hover:bg-black focus-visible:ring-2 focus-visible:ring-white"
          >
            <X size={14} />
          </button>
        )}
      ...
      <input
        ...
        onChange={(e) => e.target.files?.[0] && readImage(e.target.files[0], onChange)}
      />
    </div>
  );
}
          ...
          <div className="space-y-6">
            <section className="panel">
              ...
              <div className="space-y-5 p-4 sm:p-6">{products.map((product, index) => <div key={product.id} className="product-card">
                ...
                <div className="grid gap-4 md:grid-cols-[180px_1fr]">
                  <ImageInput id={`product-img-${product.id}`} value={product.image} label="Imagem do produto *" hint="máx. 8 MB" onChange={image => updateProduct(product.id, { image })} />
                  <div className="space-y-4">
                    <div><label htmlFor={`product-url-${product.id}`} className="label">Link do produto</label><div className="mt-2 flex gap-2"><div className="input-wrap"><Globe size={15} /><input id={`product-url-${product.id}`} type="url" value={product.url} onChange={e => updateProduct(product.id, { url: e.target.value })} placeholder="https://loja.com/produto" /></div><button onClick={() => fetchProduct(product)} disabled={!product.url || fetching === product.id} className="square-button" aria-label={fetching === product.id ? "Buscando dados do produto..." : "Buscar dados do produto"}>{fetching === product.id ? <Loader2 className="animate-spin" size={18} /> : <Zap size={18} />}</button></div></div>
                    <div><label htmlFor={`product-name-${product.id}`} className="label">Nome do produto *</label><input id={`product-name-${product.id}`} className="input mt-2" value={product.name} onChange={e => updateProduct(product.id, { name: e.target.value })} placeholder="Ex.: Liquidificador profissional 1,5 L" /></div>
                    <div className="grid gap-3 sm:grid-cols-2"><div><label htmlFor={`product-price-${product.id}`} className="label">Preço</label><div className="input-wrap mt-2"><Banknote size={15} /><input id={`product-price-${product.id}`} value={product.price} onChange={e => updateProduct(product.id, { price: e.target.value })} placeholder="R$ 4.500,00" /></div></div><div><label htmlFor={`product-badge-${product.id}`} className="label">Selo</label><select id={`product-badge-${product.id}`} className="input mt-2" value={product.badge} onChange={e => updateProduct(product.id, { badge: e.target.value as Badge })}>{BADGES.map(b => <option key={b.id} value={b.id}>{b.label}</option>)}</select></div></div>
                    <div><label htmlFor={`product-tech-${product.id}`} className="label">Informações técnicas</label><textarea id={`product-tech-${product.id}`} className="input mt-2 min-h-20 resize-y" value={product.techData} onChange={e => updateProduct(product.id, { techData: e.target.value })} placeholder="Capacidade, potência, materiais e diferenciais..." /></div>
                  </div>
                </div>
              </div>)}</div>
            </section>
            ...
            <section className="panel">
              ...
              <div className="p-4 sm:p-6">
                ...
                <div className="mt-6 grid gap-4 sm:grid-cols-2"><ImageInput id="bg-image-input" value={background} label="Cenário de referência" hint="opcional" onChange={setBackground} /><ImageInput id="logo-image-input" value={logo} label="Logo da marca" hint="opcional" onChange={setLogo} /></div>
              </div>
            </section>
            {error && <div className="error-box"><AlertCircle size={19} /><span>{error}</span><button onClick={() => setError(null)} aria-label="Fechar mensagem de erro"><X size={16} /></button></div>}
