...
function ImageInput({ value, label, hint, onChange }: { value: ImageState; label: string; hint: string; onChange: (image: ImageState) => void }) {
  ...
  return (
    <div>
      ...
      <div className="upload-zone group relative">
        <button
          type="button"
          onClick={() => input.current?.click()}
          aria-label={value.preview ? `Alterar ${label}` : `Enviar ${label}`}
          className="flex h-full w-full items-center justify-center gap-3 text-left focus:outline-none focus-visible:ring-2 focus-visible:ring-[#9fc300]"
        >
          {value.preview ? <img src={value.preview} alt={`Pré-visualização de ${label}`} className="h-full w-full object-contain p-3" /> : (
        ...
        {value.preview && (
          <button
            ...
            className="absolute right-2 top-2 rounded-full bg-black/70 p-1.5 text-white hover:bg-black/90 focus:outline-none focus-visible:ring-2 focus-visible:ring-white"
          >
            <X size={14} />
          </button>
        )}
