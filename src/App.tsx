  ...
  return (
    <div>
      ...
      <button type="button" onClick={() => input.current?.click()} className="upload-zone group">
        {value.preview ? <img src={value.preview} alt={label} className="h-full w-full object-contain p-3" /> : (
          <><span className="icon-box"><ImagePlus size={20} /></span><span><b>Enviar imagem</b><small>PNG, JPG ou WebP</small></span></>
        )}
        {value.preview && <button type="button" aria-label="Remover imagem" onClick={(e) => { e.stopPropagation(); onChange(emptyImage()); }} className="absolute right-2 top-2 rounded-full bg-black/70 p-1.5 text-white hover:bg-black focus-visible:outline-2 focus-visible:outline-amber-400"><X size={14} /></button>}
      </button>
      ...
      <main className="mx-auto max-w-[1500px] px-4 py-6 sm:px-6 lg:py-9">
        ...
        <div className="grid items-start gap-7 xl:grid-cols-[minmax(0,1.05fr)_minmax(430px,.95fr)]">
          <div className="space-y-6">
            ...
            {error && <div className="error-box"><AlertCircle size={19} /><span>{error}</span><button type="button" aria-label="Fechar mensagem de erro" onClick={() => setError(null)}><X size={16} /></button></div>}
